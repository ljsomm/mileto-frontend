...
const Modal = () => {
    ...
    return(
        <div className="modal-container" style={{ display: state.hide ? "none" : "flex"  }}>
            <div className="modal">
                <div className="modal-header">{ state.Header && <state.Header /> } { state.closeButton && <button onClick={()=>dispatch({type: "CLOSE"})} className="modal-close-button">x</button> }</div>
                ...
                )}>{ state.Body && <state.Body/> }</div>
               {
                   state.Footer && <div className={
                    classNames(
                        {
                            "modal-footer": true,
