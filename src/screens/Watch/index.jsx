...
const Watch = () => {
    ...
    async function getLastVideoWatched(id){
        try{
            ...
            setActive(data.id);
            setVideo({path: data.path, watchedTime: data.Users[0].UserVideo.watchedTime});
        }
        catch(e){
            console.log(e);
        }
    ...
    return(
        <div className={styles.grid}>
            ...
            {
                section.length > 0 ?
                section.map((item, key)=><details key={key}><summary>{key + 1}. {item.name}</summary><ol>{item.Videos.map((item, key)=>{ return <li key={item.id} className={classNames({ [styles["section-video"]]: true, [styles["section-video__active"]]: item.id === active })} onClick={()=>{ 
                    setLastVideoWatched(item.id, videoRef.current.currentTime);
                    updateVideo(item.id, item.path);
                    setActive(item.id);
                }}>{key + 1}. {item.title}</li>})}</ol></details>) 
                :
               <Loading label="Carregando Seções"/>
            }
            </div>    
            <div className={styles["video-container"]}>
                <video onSeeked={(event) => {setWatchedTime(active, event.target.currentTime)}} ref={videoRef} className={styles.video} controls>
                   {video.path !== '' && <source src={`${process.env.REACT_APP_BACKEND}${video.path.split('tmp')[1]}`} type="video/mp4"/>} 
                </video>
            </div>
