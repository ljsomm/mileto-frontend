...
const Settings = ({ title }) => {
  const settingsMenu = [
    { title: "Editar perfil", icon: "" },
    { title: "Alterar senha", icon: "" },
    { title: "Avançado", icon: "" },
  ];
  ...
  function handleRemoveImage() {
    imageRef.current.src = UserUtil.imageSrcFilter(
  ...
  async function passwordHandler(e) {
    ...
    passwordRef.current.style.borderColor =
      changePassword.password === "" ? "red" : "rgba(98, 98, 98, 0.476)";
    newPasswordRef.current.style.borderColor =
      changePassword.newPassword === "" ? "red" : "rgba(98, 98, 98, 0.476)";
    confirmationRef.current.style.borderColor =
      changePassword.confirmation === "" ? "red" : "rgba(98, 98, 98, 0.476)";
    if (
        ...
        if (res.err) {
          ...
          Modal.dispatch({
            ...
            modal: {
              ...
              Body: () => <span>Senha alterada com sucesso</span>,
              Footer: () => (
                <button
                  className="modal-button"
                  onClick={() => Modal.dispatch({ type: "CLOSE" })}
                >
                  Continuar
                </button>
              ),
  ...
  useEffect(() => {
    user.Images && setImageSrc(UserUtil.imageSrcFilter(user.Images[0].path));
  }, [user]);
      ...
      <div className={styles["settings-container"]}>
        <div className={styles["settings-menu"]}>
          <div className={styles["settings-photo"]}>
            <div className={styles["user-picture-container"]}>
              {user.Images && user.Images.length > 0 ? (
                <div className={styles["photo-container"]}>
                  <img
                    ref={imageRef}
                    src={UserUtil.imageSrcFilter(user.Images[0].path)}
                    alt="Foto do Usuário"
                  />
            ...
            <span className={styles["settings-user-name"]}>
              {user.name && UserUtil.nameFilter(user.name)}
            </span>
          </div>
            ...
            <ol>
              {settingsMenu.map((item, key) => (
                <li
                  key={key}
                  onClick={() => {
                    if (!editable) setConfigType(key + 1);
                    else alert("Salve ou cancele antes de sair");
                  }}
                  className={classNames({
                    [styles["nav-item__active"]]: configType === key + 1,
                    ...
                    <>
                      ...
                      >
                        ...
                        <input
                          type="text"
                          id="inp-gender"
                          className={styles["inp-gender"]}
                          disabled
                          value={UserUtil.genderFilter(user.Genres[0])}
                        />
                        <label>Data de Nascimento</label>
                        <div className={styles["birthdate-container"]}>
                          {user.birthDate.split("-").map((item, key) => (
                            <input
                              ref={dateRef}
                              disabled={!editable}
                              key={key}
                    ...
                    <>
                      ...
                      >
                        ...
                        <input
                          ...
                          type="password"
                          ref={confirmationRef}
                          id="inp-new-password-confirmation"
                          value={changePassword.confirmation}
                          onChange={(e) =>
                            setPassword({
                              ...changePassword,
                                    ...
                                    return (
                                      <>
                                        <form onSubmit={handlerDelete}>
                                          <p
                                            className={
                                              styles["modal-paragraph"]
                                            }
                                          >
                                            Ao realizar esta ação, serão
                                            excluídos todos os seus dados de
                                            nosso sistema, não sendo possível
                                            reverter.
                                          </p>
                                          <p
                                            className={
                                          ...
                                          >
                                            Para atestar sua confirmação digite{" "}
                                            <b>Delete</b> no campo abaixo.
                                          </p>
                                          <input
                                            name="delete"
                                            type="text"
